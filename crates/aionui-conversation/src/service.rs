...
impl ConversationService {
    ...
    async fn current_assistant_definition(
        ...
        if definition.is_some() || user_id == DEFAULT_USER_ID {
            return Ok(definition);
        }

        // Preserve assistants created before definitions became owner-scoped.
        definition_repo
            .get_by_assistant_id_for_user(DEFAULT_USER_ID, assistant_id)
            .await
            .map_err(|e| ConversationError::internal(format!("legacy assistant definition lookup failed: {e}")))
    }
    ...
    async fn resolve_assistant_snapshot(
        ...
        let (Some(_definition_repo), Some(state_repo), Some(preference_repo)) = (
            self.assistant_definition_repo(),
            self.assistant_state_repo(),
            self.assistant_preference_repo(),
        ) else {
        ...
        let Some(definition) = self.current_assistant_definition(user_id, assistant_id).await? else {
            return Ok(None);
        };
    ...
    pub async fn list(
        ...
        let hydration = self.list_hydration_context(user_id, &conversation_ids).await?;
    ...
    async fn list_hydration_context(
        ...
        let snapshots = self
            .conversation_repo
            .list_assistant_snapshots(user_id, conversation_ids)
            .await?;
        ...
        let assistant_definitions: HashMap<String, AssistantDefinitionRow> =
            if let Some(definition_repo) = self.assistant_definition_repo() {
                let mut definitions = definition_repo
                    .list_for_user(user_id)
                    .await
                    .map_err(|e| ConversationError::internal(format!("assistant definition list failed: {e}")))?;
                if user_id != DEFAULT_USER_ID {
                    let current_ids: HashSet<String> = definitions
                        .iter()
                        .map(|definition| definition.assistant_id.clone())
                        .collect();
                    definitions.extend(
                        definition_repo
                            ...
                            .await
                            .map_err(|e| {
                                ConversationError::internal(format!("legacy assistant definition list failed: {e}"))
                            })?
                            .into_iter()
                            .filter(|legacy| !current_ids.contains(&legacy.assistant_id)),
                    );
                }
                definitions
                    .into_iter()
    ...
    pub async fn transfer_owner(&self, id: &str, target_user_id: &str) -> Result<(), ConversationError> {
        ...
        self.broadcast_list_changed(target_user_id, id, "updated", None);
        Ok(())
    }
...
impl ConversationService {
    ...
    async fn backfill_extra_inplace(&self, user_id: &str, conversation_id: &str, extra: &mut serde_json::Value) {
        ...
        self.backfill_extra_inplace_with_auto_inject(user_id, conversation_id, extra, &auto_inject)
            .await;
    }
