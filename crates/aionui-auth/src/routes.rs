...
use aionui_api_types::{
    ...
    WebuiChangePasswordRequest, WebuiChangeUsernameRequest, WebuiChangeUsernameResponse, WebuiCreateUserRequest,
    WebuiCreateUserResponse, WebuiGenerateQrTokenResponse, WebuiResetPasswordResponse, WebuiResetUserPasswordResponse,
    WebuiUserResponse, WsTokenResponse,
};
...
async fn webui_list_users_handler(
    ...
    users.sort_by(|a, b| {
        ...
        b_admin.cmp(&a_admin).then_with(|| {
            a.username
                .as_deref()
                .unwrap_or_default()
                .to_lowercase()
                .cmp(&b.username.as_deref().unwrap_or_default().to_lowercase())
        })
    });
