...
#[tokio::test]
async fn migration_900021_updates_only_builtin_codex_yolo_id() {
    let pool = memory_pool().await;
    ...
    run_migration(&pool, 900021).await;
...
#[tokio::test]
async fn migration_900021_does_not_backfill_user_mode_fields() {
    let pool = memory_pool().await;
